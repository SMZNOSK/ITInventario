...
import { Monitor, RefreshCw, Plus, Trash2, Search } from "lucide-react";
...
export default function OperatingSystemsPage() {
  ...
  async function load() {
    try {
      ...
      const data = await fetchJSON<{ items: OperatingSystemApi[] }>("/api/catalog/os");
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Registra los sistemas operativos que podrás asignar a CPUs y laptops en el inventario.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <p className="mt-3 text-xs text-slate-500">
          Estos valores se usarán más adelante en la captura de inventario de CPUs y laptops.
        </p>
      </section>
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {filteredRows.length} {filteredRows.length === 1 ? "sistema" : "sistemas"} registrados
            </p>
          </div>
        ...
        <div className="divide-y divide-slate-100">
          {loading && (
            <div className="px-6 py-12 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">Cargando sistemas operativos...</p>
            </div>
          )}
          ...
          {!loading && filteredRows.length === 0 && (
            <div className="px-6 py-12 text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-slate-600">Sin sistemas operativos</h3>
              <p className="mt-1 text-sm text-slate-500">
                {search.trim() ? "No hay sistemas que coincidan con la búsqueda." : "Agrega tu primer sistema operativo arriba."}
              </p>
            </div>
          )}
