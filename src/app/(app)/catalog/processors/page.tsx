// src/app/(app)/catalog/processors/page.tsx
"use client";
...
export default function ProcessorsPage() {
  ...
  async function load() {
    try {
      ...
      const data = await fetchJSON<{ items: ProcessorApi[] }>(
      ...
      console.error(e);
      setErr(e?.message || "Error al cargar procesadores");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const body: any = {
        name: cleanName,
        nombre: cleanName,
        nombre_procesador: cleanName,
      };
      if (cleanVendor) body.vendor = cleanVendor;

      const res = await fetch("/api/catalog/processors", {
        method: "POST",
      ...
      console.error(e);
      setErr(e?.message || "No se pudo crear el procesador");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleDelete(row: ProcessorRow) {
    const confirm = window.confirm(
      `¿Eliminar definitivamente el procesador "${row.name}"? Esta acción no se puede deshacer.`,
    );
    if (!confirm) return;
    ...
    try {
      ...
      console.error(e);
      setErr(e?.message || "No se pudo eliminar el procesador");
    }
  }

  return (
    <div className="mx-auto max-w-6xl px-4 py-6 space-y-6">
      <h2 className="text-2xl font-bold tracking-tight">Procesadores</h2>
      <p className="text-sm text-slate-600">
        Registra los procesadores que se podrán asignar a CPUs y laptops en el
        inventario.
      </p>
      ...
      >
        <div className="grid gap-3 md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Intel Core i5-13400, AMD Ryzen 5 5600G…"
              className="w-full rounded-lg border border-slate-300 bg-slate-900 text-slate-50
        ...
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex items-center rounded-lg bg-slate-900 px-4 py-2 text-sm
                       font-semibold text-white shadow-sm hover:bg-slate-800
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td
                  ...
                  className="px-4 py-6 text-center text-slate-500"
                >
                  Aún no hay procesadores registrados.
                </td>
              </tr>
            )}
            ...
            {!loading &&
              rows.map((row) => (
                ...
                >
                  <td className="px-4 py-3 whitespace-nowrap text-slate-900 dark:text-slate-900">
                    {row.name || (
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <button
                      ...
                      className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold
                        ${statusStyles[row.active ? "true" : "false"]}`}
                    >
