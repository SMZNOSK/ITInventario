...
import { Cpu, RefreshCw, Plus, Trash2, Search } from "lucide-react";
...
export default function ProcessorsPage() {
  ...
  async function load() {
    try {
      ...
      const data = await fetchJSON<{ items: ProcessorApi[] }>("/api/catalog/processors");
      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Registra los procesadores que se podrán asignar a CPUs y laptops en el inventario.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {filteredRows.length} {filteredRows.length === 1 ? "procesador" : "procesadores"} registrados
            </p>
          </div>
        ...
        <div className="divide-y divide-slate-100">
          {loading && (
            <div className="px-6 py-12 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">Cargando procesadores...</p>
            </div>
          )}
          ...
          {!loading && filteredRows.length === 0 && (
            <div className="px-6 py-12 text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-slate-600">Sin procesadores</h3>
              <p className="mt-1 text-sm text-slate-500">
                {search.trim() ? "No hay procesadores que coincidan con la búsqueda." : "Agrega tu primer procesador arriba."}
              </p>
            </div>
          )}
