...
import { MemoryStick, RefreshCw, Plus, Trash2, Search } from "lucide-react";
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-800 flex items-center gap-3">
            <MemoryStick className="w-7 h-7 text-indigo-600" />
            Memoria RAM
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Registra las configuraciones de memoria RAM que se podrán asignar a CPUs y laptops en el inventario.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">
          Agregar nueva memoria RAM
        </h2>
        <div className="flex gap-3">
          <div className="relative flex-1">
            <MemoryStick className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input
              type="text"
              ...
              placeholder="Descripción de la memoria RAM"
              value={name}
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {filteredRows.length} {filteredRows.length === 1 ? "configuración" : "configuraciones"} registradas
            </p>
          </div>
        ...
        <div className="divide-y divide-slate-100">
          {loading && (
            <div className="px-6 py-12 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">Cargando memorias RAM...</p>
            </div>
          )}

          {!loading && filteredRows.length === 0 && (
            <div className="px-6 py-12 text-center">
              <MemoryStick className="w-12 h-12 text-slate-300 mx-auto" />
              <h3 className="mt-4 text-lg font-medium text-slate-600">Sin memorias RAM</h3>
              <p className="mt-1 text-sm text-slate-500">
                {search.trim() ? "No hay memorias que coincidan con la búsqueda." : "Agrega tu primera memoria RAM arriba."}
              </p>
            </div>
          )}
