// src/app/api/transfers/assets/[id]/confirm/route.ts
...
/**
 * POST /api/transfers/assets/:id/confirm
 * Confirmar una transferencia (solo usuarios con acceso al hotel destino)
 */
export const POST = withError(async (req: NextRequest, { params }: RouteParams) => {
    ...
    // 5. Verificar acceso al hotel destino (solo el destino puede confirmar)
    if (auth.data.role !== "ADMIN") {
        const hotelDeny = ensureHotelAccess(auth.data, transfer.destHotelId, "hotel destino");
        if (hotelDeny) return hotelDeny;
    }

    // 6. Confirmar transferencia
    try {
        const confirmed = await transfersService.confirm(transferId, auth.data.id);
        ...
        return NextResponse.json({
            ...
            transfer: {
                ...
                acceptedAt: confirmed.acceptedAt,
            },
        });
    } catch (err: any) {
