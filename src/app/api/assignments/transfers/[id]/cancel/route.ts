// src/app/api/assignments/transfers/[id]/cancel/route.ts
...
/**
 * POST /api/assignments/transfers/:id/cancel
 * Cancela una transferencia de asignaciones pendiente.
 */
export const POST = withError(
    async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
        ...
        if (auth.data.role !== "ADMIN") {
            ...
            if (!hasAccessOrigin && !hasAccessDest) {
                return NextResponse.json(
                    {
                        error:
                            "No tienes permisos para cancelar esta transferencia.",
                    },
                    { status: 403 }
                );
            }
        ...
        const updatedTransfer = await prisma.assignmentTransfer.update({
            where: { id: transferId },
            data: {
                status: "CANCELED",
